...
if (isset($_POST['submit'])) {
    ...
    // Memeriksa validasi
    if ($valid) {
        // TODO 3: Buatlah query untuk melakukan verifikasi terhadap kredensial yang diberikan // .md5($password). // halo = 57f842286171094855e51fc3a541c1e2 -> password di database
        $query = "SELECT * FROM admin WHERE email='".$email."' AND password='".md5($password)."' ";
        // TODO 4: Eksekusi query
        $result = $db->query($query);
        if (!$result){
            die("Could not query the databse: <br/>".$db->error);
        } else {
